...
{
	global $pagenow;
	if (get_option('mgelementorp') || $pagenow == 'themes.php') {
		return;
	}
	$class = 'eye-notice notice notice-warning is-dismissible';
	$message = __('<strong>Upgrade Pro!! upgrade the Magic Elementor Pro theme to get the next level of experience.!!<br /> Only active the Pro theme and install demo only one click then your site will be ready with lots of advanced features. Now create your site with the pro version!!!! So why late get pro now.</strong> ', 'magic-elementor-lite');
	$url1 = esc_url('https://mgpro.fullsitediting.com/demos');
	$url3 = esc_url('https://wpthemespace.com/product/magic-elementor-pro/?add-to-cart=7944');

	printf('<div class="%1$s" style="padding:10px 15px 20px;text-transform:uppercase"><p>%2$s</p><a target="_blank" class="button button-primary" href="%3$s" style="margin-right:10px">' . __('Magic Elementor Pro Details', 'magic-elementor-lite') . '</a><a target="_blank" class="button button-primary" href="%4$s" style="margin-right:10px">' . __('Upgrade Pro', 'magic-elementor-lite') . '</a><button class="button button-info btnend" style="margin-left:10px">' . __('Dismiss the notice', 'magic-elementor-lite') . '</button></div>', esc_attr($class), wp_kses_post($message), $url1, $url3);
}
