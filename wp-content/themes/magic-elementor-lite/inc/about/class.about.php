 ...
 * @since 1.0.0
 */
if (!class_exists('magic_elementor_lite_About')) {
	/**
	 * Singleton class used for generating the about page of the theme.
	 */
	class magic_elementor_lite_About
	{
		/**
		 ...
		 *
		 * @var magic_elementor_lite_About $instance The  TI_About_Page instance.
		 */
		...
		{
			if (!isset(self::$instance) && !(self::$instance instanceof magic_elementor_lite_About)) {
				self::$instance = new magic_elementor_lite_About;
		...
		public function ti_about_page_render()
		{
			...
			if (!empty($welcome_title) || !empty($welcome_content) || !empty($this->tabs)) {
				...
				echo '<a href="' . esc_url('https://afx.fullsitediting.com/demos') . '" target="_blank" class="wp-badge epsilon-welcome-logo"></a>';
...
<?php
		...
		private function parse_changelog()
		{
			...
			$changelog = $wp_filesystem->get_contents(get_stylesheet_directory() . '/CHANGELOG.md');
		...
		public function free_pro()
		{
			...
			if (!empty($free_pro)) {
				if (!empty($free_pro['free_theme_name']) && !empty($free_pro['pro_theme_name']) && !empty($free_pro['features']) && is_array($free_pro['features'])) {
					...
					echo '<tr><td style="border-top:0;"></td><td style="border-top:0;"></td><td style="border-top:0;" class="only-pro"><a href="' . esc_url('https://afx.fullsitediting.com/demos') . '" target="_blank" >';
					echo esc_html('Magic Elementor Pro Live Preview', 'magic-elementor-lite');
					echo '</a></td></tr>';
		...
		public function style_and_scripts($hook_suffix)
		{
			...
			wp_enqueue_style('about-page-css', get_stylesheet_directory_uri() . '/inc/about/css/about.min.css', array(), '2.3.4', 'all');
			wp_enqueue_script('eye-notice-js', get_stylesheet_directory_uri() . '/inc/about/js/notice.js', array('jquery'), '2.3.2', true);
			...
			if ('appearance_page_' . $this->theme_slug . '-welcome' == $hook_suffix) {
				...
				wp_localize_script(
					...
					array(
						...
						'activating_string'        => __('Activating', 'magic-elementor-lite')
					)
				);
			}
		}
